...
def polarity(tweet, sentDict):
    ...
    for w in tweet.split():
        ...
        if hashTest(w) and w[1:] in sentDict.keys():
            pos = pos + 2 * sentDict[w[1:]][1]
...
def posPolarity(tweet, sentDict):
    pos = 0.0
    neg = 0.0
    neu = 0.0
    ...
    for z in tags:
        ...
        if hashTest(list(w)[0]) and (list(w)[0][1:], list(w)[1]) in sentDict.keys():
            n_words = n_words + 1
            posNumber[w[1]] += 1
    ...
    if (n_words == 0):
        return [pos, neg, neu, posScores['a'], posScores['n'], posScores['v'], posScores['r'], posNumber['a'],
                posNumber['n'], posNumber['v'], posNumber['r']]
    else:
        return [pos / n_words, neg / n_words, neu / n_words, posScores['a'] / n_words, posScores['n'] / n_words,
                posScores['v'] / n_words, posScores['r'] / n_words, posNumber['a'] / n_words, posNumber['n'] / n_words,
                posNumber['v'] / n_words,
                posNumber['r'] / n_words]  # do not use the neutral scoore it will fuck up evertg
